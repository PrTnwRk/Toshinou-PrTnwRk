class StatisticWindow {
    ...
    createWindow() {
        ...
        $(window).on('logicEnd', ()=> {
            if (this.connected) {
				...
				$('span:last-child', this.cspeed).text(this.cspeedFormat(this.stats.credits, this.stats.startTime));
            }
        });
    }
    ...
    speedFormat(uri, startTime){
        ...
        let result = uri>0 & timeMinutes>0 ? formats[curFormat]:'0';
        result += `  uri/${curFormat}`;

        return result;
    }
	
	cspeedFormat(credits, startTime) {

		let timeMinutes = TimeHelper.totalMinutes(startTime);
		let curFormat = window.globalSettings.speedFormat;

		let formats = {
		min: (credits / timeMinutes).toFixed(2),
		hour: ((credits / timeMinutes) * 60).toFixed(2),
		};

		let cresult = credits > 0 & timeMinutes > 0 ? formats[curFormat] : '0';
		cresult += `  credits/${curFormat}`;
